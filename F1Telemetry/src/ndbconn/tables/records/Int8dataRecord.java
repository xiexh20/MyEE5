/*
 * This file is generated by jOOQ.
 */
package ndbconn.tables.records;


import ndbconn.tables.Int8data;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record5;
import org.jooq.Row5;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class Int8dataRecord extends UpdatableRecordImpl<Int8dataRecord> implements Record5<Integer, Byte, Short, Integer, Integer> {

    private static final long serialVersionUID = 1626710690;

    /**
     * Setter for <code>NewF1DB.Int8Data.idData</code>.
     */
    public void setIddata(Integer value) {
        set(0, value);
    }

    /**
     * Getter for <code>NewF1DB.Int8Data.idData</code>.
     */
    public Integer getIddata() {
        return (Integer) get(0);
    }

    /**
     * Setter for <code>NewF1DB.Int8Data.data</code>.
     */
    public void setData(Byte value) {
        set(1, value);
    }

    /**
     * Getter for <code>NewF1DB.Int8Data.data</code>.
     */
    public Byte getData() {
        return (Byte) get(1);
    }

    /**
     * Setter for <code>NewF1DB.Int8Data.dataName</code>.
     */
    public void setDataname(Short value) {
        set(2, value);
    }

    /**
     * Getter for <code>NewF1DB.Int8Data.dataName</code>.
     */
    public Short getDataname() {
        return (Short) get(2);
    }

    /**
     * Setter for <code>NewF1DB.Int8Data.packetId</code>.
     */
    public void setPacketid(Integer value) {
        set(3, value);
    }

    /**
     * Getter for <code>NewF1DB.Int8Data.packetId</code>.
     */
    public Integer getPacketid() {
        return (Integer) get(3);
    }

    /**
     * Setter for <code>NewF1DB.Int8Data.sessionId</code>.
     */
    public void setSessionid(Integer value) {
        set(4, value);
    }

    /**
     * Getter for <code>NewF1DB.Int8Data.sessionId</code>.
     */
    public Integer getSessionid() {
        return (Integer) get(4);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    @Override
    public Record1<Integer> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record5 type implementation
    // -------------------------------------------------------------------------

    @Override
    public Row5<Integer, Byte, Short, Integer, Integer> fieldsRow() {
        return (Row5) super.fieldsRow();
    }

    @Override
    public Row5<Integer, Byte, Short, Integer, Integer> valuesRow() {
        return (Row5) super.valuesRow();
    }

    @Override
    public Field<Integer> field1() {
        return Int8data.INT8DATA.IDDATA;
    }

    @Override
    public Field<Byte> field2() {
        return Int8data.INT8DATA.DATA;
    }

    @Override
    public Field<Short> field3() {
        return Int8data.INT8DATA.DATANAME;
    }

    @Override
    public Field<Integer> field4() {
        return Int8data.INT8DATA.PACKETID;
    }

    @Override
    public Field<Integer> field5() {
        return Int8data.INT8DATA.SESSIONID;
    }

    @Override
    public Integer component1() {
        return getIddata();
    }

    @Override
    public Byte component2() {
        return getData();
    }

    @Override
    public Short component3() {
        return getDataname();
    }

    @Override
    public Integer component4() {
        return getPacketid();
    }

    @Override
    public Integer component5() {
        return getSessionid();
    }

    @Override
    public Integer value1() {
        return getIddata();
    }

    @Override
    public Byte value2() {
        return getData();
    }

    @Override
    public Short value3() {
        return getDataname();
    }

    @Override
    public Integer value4() {
        return getPacketid();
    }

    @Override
    public Integer value5() {
        return getSessionid();
    }

    @Override
    public Int8dataRecord value1(Integer value) {
        setIddata(value);
        return this;
    }

    @Override
    public Int8dataRecord value2(Byte value) {
        setData(value);
        return this;
    }

    @Override
    public Int8dataRecord value3(Short value) {
        setDataname(value);
        return this;
    }

    @Override
    public Int8dataRecord value4(Integer value) {
        setPacketid(value);
        return this;
    }

    @Override
    public Int8dataRecord value5(Integer value) {
        setSessionid(value);
        return this;
    }

    @Override
    public Int8dataRecord values(Integer value1, Byte value2, Short value3, Integer value4, Integer value5) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached Int8dataRecord
     */
    public Int8dataRecord() {
        super(Int8data.INT8DATA);
    }

    /**
     * Create a detached, initialised Int8dataRecord
     */
    public Int8dataRecord(Integer iddata, Byte data, Short dataname, Integer packetid, Integer sessionid) {
        super(Int8data.INT8DATA);

        set(0, iddata);
        set(1, data);
        set(2, dataname);
        set(3, packetid);
        set(4, sessionid);
    }
}
